...
if (Sys.info()['sysname'] == "Linux") os <- "linux-devragg"
...
# Example 1 - png
png(glue('img/plotmath-ex1-{os}-png.png'), units = "px", res = reso, height = scl*480, width = scl*480)
x <- seq(-4, 4, length.out = 101)
y <- cbind(sin(x), cos(x))
matplot(x, y, type = "l", xaxt = "n",
        main = expression(paste(plain(sin) * phi, "  and  ",
...
# Example 2 - png
## How to combine "math" and numeric variables :
png(glue('img/plotmath-ex2-{os}-png.png'), units = "px", res = reso, height = scl*480, width = scl*480)
plot(1:10, type="n", xlab="", ylab="", main = "plot math & numbers")
theta <- 1.23 ; mtext(bquote(hat(theta) == .(theta)), line= .25)
for(i in 2:9)
  text(i, i+1, substitute(list(xi, eta) == group("(",list(x,y),")"),
...
# Example 3 - png
## note the "{ .. }" trick to get "chained" equations:
png(glue('img/plotmath-ex3-{os}-png.png'), units = "px", res = reso, height = scl*480, width = scl*480)
plot(1:10, 1:10, main = quote(1 <= {1 < 2}))
text(4, 9, expression(hat(beta) == (X^t * X)^{-1} * X^t * y))
text(4, 8.4, "expression(hat(beta) == (X^t * X)^{-1} * X^t * y)",
     cex = .8)
...
# Example 4 - ragg
## some other useful symbols
png(glue('img/plotmath-ex4-{os}-png.png'), units = "px", res = reso, height = scl*480, width = scl*480)
plot.new(); plot.window(c(0,4), c(15,1))
text(1, 1, "universal", adj = 0); text(2.5, 1,  "\\042")
text(3, 1, expression(symbol("\042")))
text(1, 2, "existential", adj = 0); text(2.5, 2,  "\\044")
